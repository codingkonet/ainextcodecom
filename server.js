...
const systemPrompt = `You are a helpful AI assistant for a multi-model SaaS platform. Be clear, accurate, concise, practical, and honest about uncertainty.`;
...
const seed = {
  ...
  payments: [],
  plans: [
    { id: "free", name: "Free", price: 0, currency: "USD", interval: "month", messageLimit: 25, providerAccess: ["openai"], features: ["OpenAI chat", "Basic themes", "Community plugins"], active: true },
    { id: "pro", name: "Pro", price: 19, currency: "USD", interval: "month", messageLimit: 500, providerAccess: ["openai", "gemini", "claude"], features: ["All providers", "More messages", "Premium themes"], active: true },
    { id: "business", name: "Business", price: 49, currency: "USD", interval: "month", messageLimit: 2500, providerAccess: ["openai", "gemini", "claude"], features: ["Team-ready limits", "Admin controls", "Priority setup"], active: true }
  ],
  plugins: [
...
function json(res, status, body, headers = {}) { const out = JSON.stringify(body); res.writeHead(status, { "Content-Type": "application/json; charset=utf-8", "Content-Length": Buffer.byteLength(out), ...headers }); res.end(out); }
...
function safeUser(user) { if (!user) return null; const { passwordHash, apiKeys, ...rest } = user; return { ...rest, modelPrefs: user.modelPrefs || {}, apiKeyStatus: { openai: !!apiKeys?.openai, gemini: !!apiKeys?.gemini, claude: !!apiKeys?.claude } }; }
...
function config(data = db(), user = null) { return { providers: providerList(user), plans: data.plans.filter(x => x.active), plugins: data.plugins.filter(x => x.active), themes: data.themes.filter(x => x.active), paypal: { configured: !!(paypal.clientId && paypal.secret), clientId: paypal.clientId || null, mode: paypal.mode } }; }
...
async function apiSettings(req, res) { const ctx = needUser(req, res); if (!ctx) return; const input = await body(req); ctx.user.apiKeys ||= {}; ctx.user.modelPrefs ||= {}; for (const p of ["openai", "gemini", "claude"]) { if (input.clear?.[p]) delete ctx.user.apiKeys[p]; const key = String(input[`${p}Key`] || "").trim(); if (key) ctx.user.apiKeys[p] = key; const model = String(input.models?.[p] || "").trim(); if (model) ctx.user.modelPrefs[p] = model; } save(ctx.data); json(res, 200, { user: safeUser(ctx.user), config: config(ctx.data, ctx.user) }); }
async function chat(req, res) { const ctx = needUser(req, res); if (!ctx) return; const input = await body(req); const data = ctx.data; const user = ctx.user; const plan = planOf(data, user); const provider = ["openai", "gemini", "claude"].includes(input.provider) ? input.provider : "openai"; if (!plan.providerAccess.includes(provider)) return json(res, 403, { error: `${plan.name} does not include ${provider}.` }); const month = new Date().toISOString().slice(0, 7); user.usage ||= {}; user.usage[month] ||= 0; if (user.usage[month] >= plan.messageLimit) return json(res, 402, { error: "Monthly message limit reached." }); const messages = cleanMessages(input.messages); const model = input.model || user.modelPrefs?.[provider] || providerList(user).find(p => p.id === provider).defaultModel; const key = user.apiKeys?.[provider] || serverKeys[provider]; const prompt = instructions(data, input.specialization, input.plugins || []); const answer = provider === "gemini" ? await callGemini(key, model, prompt, messages) : provider === "claude" ? await callClaude(key, model, prompt, messages) : await callOpenAI(key, model, prompt, messages); user.usage[month] += 1; save(data); json(res, 200, { message: answer, provider, model, usage: user.usage[month], limit: plan.messageLimit }); }
async function paypalOrder(req, res) { const ctx = needUser(req, res); if (!ctx) return; const input = await body(req); const plan = ctx.data.plans.find(p => p.id === input.planId); if (!plan) return json(res, 404, { error: "Plan not found." }); if (plan.price <= 0) { ctx.user.planId = plan.id; save(ctx.data); return json(res, 200, { free: true, planId: plan.id }); } if (!paypal.clientId || !paypal.secret) return json(res, 500, { error: "PayPal is not configured." }); json(res, 501, { error: "Add PayPal Orders API credentials and webhook handling before production checkout." }); }
async function adminOverview(req, res) { const ctx = needAdmin(req, res); if (!ctx) return; json(res, 200, { users: ctx.data.users.map(safeUser), plans: ctx.data.plans, plugins: ctx.data.plugins, themes: ctx.data.themes, payments: ctx.data.payments, providers: providerList(), paypalConfigured: !!(paypal.clientId && paypal.secret) }); }
...
async function adminAdd(req, res, type) { const ctx = needAdmin(req, res); if (!ctx) return; const input = await body(req); const collection = ctx.data[type]; const idValue = String(input.id || input.name || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""); if (!idValue || !input.name) return json(res, 400, { error: "Id and name required." }); const item = type === "plans" ? { id: idValue, name: input.name, price: Number(input.price || 0), currency: "USD", interval: "month", messageLimit: Number(input.messageLimit || 100), providerAccess: input.providerAccess || ["openai"], features: String(input.features || "").split("\n").filter(Boolean), active: true } : type === "themes" ? { id: idValue, name: input.name, active: true } : { id: idValue, name: input.name, description: input.description || "", prompt: input.prompt || "", active: true }; const i = collection.findIndex(x => x.id === idValue); if (i >= 0) collection[i] = item; else collection.push(item); save(ctx.data); json(res, 200, { item }); }
...
ensureDb();
http.createServer((req, res) => Promise.resolve(route(req, res)).catch(err => json(res, 500, { error: err.message || "Server error." }))).listen(PORT, () => console.log(`ModelDesk AI running on http://localhost:${PORT}`));
